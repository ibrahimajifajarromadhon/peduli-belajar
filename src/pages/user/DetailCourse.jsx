...
const DetailCourse = () => {
  ...
  const handleVideoEnded = () => {
    ...
    const currentSubject = dataCourse.chapter[currentChapterIndex].subject[currentSubjectIndex];
    sendVideoProgress(currentSubject.subjectId);
  ...
  const sendVideoProgress = async (subjectId) => {
    try {
      const token = Cookies.get("token");
      const apiUrl = `${import.meta.env.VITE_API}/api/user/progress`;
          ...
          >
            <div style={{ margin: "15px" }}>
              <div className="row">
                ...
                <div className="col-5">
                <div className="progress mt-2" style={{ width: "150px", height: "16px", top: "1px", left: "16px", borderRadius: "25px" }}>
  <div
    className="progress-bar"
    style={{
      width: `${(watchedSubjects.filter(Boolean).length / dataCourse.chapter.reduce((total, chapter) => total + chapter.subject.length, 0)) * 100}%`,
      background: "#6148FF",
      borderRadius: "25px",
      fontWeight: "600",
    }}
  >
    {`${Math.floor((watchedSubjects.filter(Boolean).length / dataCourse.chapter.reduce((total, chapter) => total + chapter.subject.length, 0)) * 100)}% complete`}
  </div>
</div>
...
<button
  ...
  disabled={currentSubjectIndex === dataCourse.chapter.length - 1}
>
  Next Video
</button>
